// $Id: subscriptions_tableselect.js,v 1.1.2.2 2009/04/13 15:26:24 salvis Exp $
...
Drupal.subscriptions_tableSelect = function() {
  ...
  var table = this, selectAll, checkboxes, lastChecked, settings = Drupal.settings.subscriptions_tableSelect;
  var updateSelectAll = function(state) {
    selectAll[0].checked = state;
    // Set the title to the current action.
    selectAll.attr('title', state ? settings.selectNone : settings.selectAll);
  };
...
if (Drupal.jsEnabled) {
  $(document).ready(function() {
    $('form table[th.subscriptions-table]').each(Drupal.subscriptions_rowSelect);
    $('form table[th.select-all]').each(Drupal.subscriptions_tableSelect);
  });
}
